...
func assertFile(
	...
	if ch, ok = gm[filename]; !ok {
		t.Fatal(filename)
	}

	if ch[0].AbbreviatedHash != expectedAbbreviatedHash || ch[0].Hash != expectedHash {
		t.Error("Invalid tree hash, file", filename, "abbreviated:", ch[0].AbbreviatedHash, "full:", ch[0].Hash, ch[0].Subject)
	}

	if ch[0].AuthorName != "Bjørn Erik Pedersen" && ch[0].AuthorName != "Michael Stapelberg" {
		t.Error("These commits are mine! Got", ch[0].AuthorName, "and", ch[0].AuthorEmail)
	}

	if ch[0].AuthorEmail != "bjorn.erik.pedersen@gmail.com" && ch[0].AuthorEmail != "stapelberg@google.com" {
		t.Error("These commits are mine! Got", ch[0].AuthorName, "and", ch[0].AuthorEmail)
	}

	if got, want := ch[0].AuthorDate.Format("2006-01-02"), expectedAuthorDate; got != want {
		t.Errorf("%s: unexpected author date: got %v, want %v", filename, got, want)
	}

	if got, want := ch[0].CommitDate.Format("2006-01-02"), expectedCommitDate; got != want {
...
// 	}
//
// 	expected := "/nickanderegg/gitchangelog"
//
